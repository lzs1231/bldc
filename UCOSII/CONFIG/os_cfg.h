...
#define OS_TICKS_PER_SEC       	250u   /* Set the number of ticks in one second  在一秒内设置节拍数，每次间隔5ms产生一次节拍中断     */
...
#define OS_TASK_SW_HOOK_EN        1u   /*     Include code for OSTaskSwHook()                          */
                                       ...
                                       /* ----------------------- EVENT FLAGS ------------------------ */
#define OS_FLAG_EN                1u   /* Enable (1) or Disable (0) code generation for EVENT FLAGS  事件标志启用（1）或禁用（0）代码生成  */
...
#define OS_FLAGS_NBITS           16u   /* Size in #bits of OS_FLAGS data type (8, 16 or 32)            */
                                       ...
                                       /* -------------------- MESSAGE MAILBOXES --------------------- */
#define OS_MBOX_EN                1u   /* Enable (1) or Disable (0) code generation for MAILBOXES   邮箱启用（1）或禁用（0）代码生成  */
...
#define OS_MBOX_QUERY_EN          1u   /*     Include code for OSMboxQuery()                           */
                                       ...
                                       /* --------------------- MEMORY MANAGEMENT -------------------- */
#define OS_MEM_EN                 1u   /* Enable (1) or Disable (0) code generation for MEMORY MANAGER 内存管理器启用（1）或禁用（0）代码生成*/
#define OS_MEM_NAME_EN            1u   /*     Enable memory partition names                            */
#define OS_MEM_QUERY_EN           1u   /*     Include code for OSMemQuery()                            */


                                       /* ---------------- MUTUAL EXCLUSION SEMAPHORES --------------- */
#define OS_MUTEX_EN               1u   /* Enable (1) or Disable (0) code generation for MUTEX      互斥体启用（1）或禁用（0）代码生成  */
...
#define OS_MUTEX_QUERY_EN         1u   /*     Include code for OSMutexQuery()                          */
                                       ...
                                       /* ---------------------- MESSAGE QUEUES ---------------------- */
#define OS_Q_EN                   1u   /* Enable (1) or Disable (0) code generation for QUEUES     队列启用（1）或禁用（0）代码生成  */
...
#define OS_Q_QUERY_EN             1u   /*     Include code for OSQQuery()                              */
                                       ...
                                       /* ------------------------ SEMAPHORES ------------------------ */
#define OS_SEM_EN                 1u   /* Enable (1) or Disable (0) code generation for SEMAPHORES   信号量启用（1）或禁用（0）代码生成  */
