...
#define  OS_GLOBALS      //如果定义了OS_GLOBALS，就将OS_EXT定义为空 
#include <ucos_ii.h>     
#endif
...
void  OSInit (void)
{
    ...
    OS_InitMisc();                                               /* Initialize miscellaneous variables  初始化其他变量     */

    OS_InitRdyList();                                            /* Initialize the Ready List    初始化就绪列表             */
    ...
    OS_FlagInit();                                               /* Initialize the event flag structures 初始化事件标志结构    */
#endif

#if (OS_MEM_EN > 0u) && (OS_MAX_MEM_PART > 0u)
    OS_MemInit();                                                /* Initialize the memory manager  初始化内存管理器          */
#endif
...
#if OS_TMR_EN > 0u
    OSTmr_Init();                                                /* Initialize the Timer Manager  初始化计时器管理器           */
#endif
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        OSStartHighRdy();                            /* Execute target specific code to start task  执行特定于目标的代码以启动任务   */
    }
}
/*$PAGE*/
/*
...
void  OSStatInit (void)
{
    ...
    OS_CPU_SR  cpu_sr = 0u;
 	...
 	//OSStatInit()必须在时钟节拍启动之后调用；否则，用户的应用程序就会崩溃。
    OSTimeDly(2u);                               /* Synchronize with clock tick 将自身延时了两个时钟节拍，这样它就可以与时钟节拍中断同步    */
    OS_ENTER_CRITICAL();											
    OSIdleCtr    = 0uL;                          /* Clear idle counter                                 */
    OS_EXIT_CRITICAL();
    OSTimeDly(OS_TICKS_PER_SEC / 10u);           /* Determine MAX. idle counter value for 1/10 second    统计任务挂起，只能执行空闲任务，统计0.1秒内 OSIdleCtr递增数量*/
    OS_ENTER_CRITICAL();
...
void  OSTimeTick (void)       //每个节拍调用一次该函数
{
    OS_TCB    *ptcb;
    ...
    OS_ENTER_CRITICAL();                                   
    OSTime++;                                              /* Update the 32-bit tick counter   更新32位嘀嗒计数器            */
    OS_EXIT_CRITICAL();
#endif
    if (OSRunning == OS_TRUE) {                            //如果os已开始运行（OSStart()已执行，则执行if里面的语句;
#if OS_TICK_STEP_EN > 0u
...
void  OS_Sched (void)
{
    ...
    OS_ENTER_CRITICAL();
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...     仅当所有ISR完成且计划程序未锁定   */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            OS_SchedNew();
            OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy   如果当前任务的RDY最高，则没有CTX软件  */
#if OS_TASK_PROFILE_EN > 0u
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task    Inc.#的上下文切换到此任务 */
#endif
                OSCtxSwCtr++;                          /* Increment context switch counter           增量上下文切换计数器  */
                OS_TASK_SW();                          /* Perform a context switch                   执行上下文切换  */
            }
        }
    }
    OS_EXIT_CRITICAL();
...
void  OS_TaskIdle (void *p_arg) 
{
    ...
    for (;;) {
        ...
        OSTaskIdleHook();                        /* Call user definable HOOK        空闲任务钩子程序       */
    }
}
/*$PAGE*/
/*
*********************************************************************************************************
*                                            STATISTICS TASK    统计任务
*
...
*
*                                          OSIdleCtr
*                 OSCPUUsage = 100 * (1 - ------------)     (units are in %利用空闲计数器求出CPU有多少时间在空闲任务中，即有多少CPU占空比。)
*                                         OSIdleCtrMax
*
...
#endif
        OSTimeDly(OS_TICKS_PER_SEC / 10u);       /* Accumulate OSIdleCtr for the next 1/10 second  在接下来的1/10秒内累积Osidletrc    */
    }
}
#endif
/*$PAGE*/
