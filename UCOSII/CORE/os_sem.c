...
OS_EVENT  *OSSemCreate (INT16U cnt)
{
    ...
    if (pevent != (OS_EVENT *)0) {                         /* Get an event control block               */
        ...
        pevent->OSEventCnt     = cnt;                      /* Set semaphore value     设置信号量值      */
                 ...
                 INT8U     *perr)
{
...
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_SEM) {   /* Validate event block type   验证事件块类型                  */
        *perr = OS_ERR_EVENT_TYPE;
        return;
    }
    if (OSIntNesting > 0u) {                          /* See if called from ISR ...                    */
    ...
    OS_EventTaskWait(pevent);                         /* Suspend task until event or timeout occurs    暂停任务直到事件或超时发生*/
    OS_EXIT_CRITICAL();
