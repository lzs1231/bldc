                   ...
                   INT8U     *perr)
{
    ...
    OSTCBCur->OSTCBDly       = timeout;               /* Load timeout in TCB                       TCB中的加载超时    */
    OS_EventTaskWait(pevent);                         /* Suspend task until event or timeout occurs    在事件或超时发生之前挂起任务*/
    OS_EXIT_CRITICAL();
    OS_Sched();                                       /* Find next highest priority task ready to run  查找下一个最高优先级的任务以准备运行 */
    OS_ENTER_CRITICAL();
    switch (OSTCBCur->OSTCBStatPend) {                /* See if we timed-out or aborted                看看我们是超时还是中止 */
                   ...
                   void      *pmsg)
{
...
#if OS_ARG_CHK_EN > 0u
    if (pevent == (OS_EVENT *)0) {                    /* Validate 'pevent'                             */
        return (OS_ERR_PEVENT_NULL);                  /* pevent为空错误 */
    }
    if (pmsg == (void *)0) {                          /* Make sure we are not posting a NULL pointer   */
        return (OS_ERR_POST_NULL_PTR);                /* pmsg为空错误 */
    }
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_MBOX) {  /* Validate event block type                     */
    ...
    OS_ENTER_CRITICAL();
    if (pevent->OSEventGrp != 0u) {                   /* See if any task pending on mailbox  检查是否有任务在等待该邮箱中的消息，OSEventGrp 域包含非零值，就暗示着有任务在等待该消息         */
                                                      /* Ready HPT waiting on event  	*/
			//调用OSEventTaskRdy()将其中的最高优先级任务从等待列表中删除，使等待消息任务进入就绪状态，加入系统的就绪任务列表中，准备运行
        (void)OS_EventTaskRdy(pevent, pmsg, OS_STAT_MBOX, OS_STAT_PEND_OK);  
        OS_EXIT_CRITICAL();           //退出“临界区”的功能代码,开中断，****错误地方
		OS_Sched();                                   /* Find highest priority task ready to run       */
        return (OS_ERR_NONE);
    }
    if (pevent->OSEventPtr != (void *)0) {            /* Make sure mailbox doesn't already have a msg  */
        OS_EXIT_CRITICAL();                           /* 消息邮箱有其他消息 */
        return (OS_ERR_MBOX_FULL);
    }
    pevent->OSEventPtr = pmsg;                        /* Place message in mailbox  将消息放入邮箱       */
    OS_EXIT_CRITICAL();
    return (OS_ERR_NONE);
}
#endif
