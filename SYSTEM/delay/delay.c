...
#if SYSTEM_SUPPORT_OS							//如果SYSTEM_SUPPORT_OS=1定义了,说明要支持OS了(不限于UCOS).
...
#define delay_ostickspersec	OS_TICKS_PER_SEC	//OS时钟节拍,即每秒调度次数200
...
void delay_init()
{
	...
	fac_us=SystemCoreClock/8000000;				//为系统时钟的1/8 = 9
#if SYSTEM_SUPPORT_OS  							//如果需要支持OS.
	reload=SystemCoreClock/8000000;				//每秒钟的计数次数 单位为M  
	reload*=1000000/delay_ostickspersec;		//根据delay_ostickspersec设定溢出时间    reload= 45000  
												//reload为24位寄存器,最大值:16777216,在72M下,约合1.86s左右	
	fac_ms=1000/delay_ostickspersec;			//代表OS可以延时的最少单位	   

	SysTick->CTRL|=SysTick_CTRL_TICKINT_Msk;   	//开启SYSTICK中断
	SysTick->LOAD=reload; 						          //每1/delay_ostickspersec=5ms秒中断一次	
