...
u8     RS232_RX_BUFF[CMD_MAX_SIZE];          //DMA目标地址,内存地址    接收缓冲区
u8     RS232_TX_BUFF[CMD_MAX_SIZE];          //DMA目标地址,内存地址    发送缓冲区
...
void uart3_init(u32 BaudRate)
{
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)&RS232_RX_BUFF;           //内存地址
...
//	DMA_ITConfig(DMA1_Channel3, DMA_IT_TC, ENABLE);           //DMA通道1传输完成中断
	...
	DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&(USART3->DR));  //ADC地址  (uint32_t)(&(ADC1->DR))
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)&RS232_TX_BUFF;        //内存地址
...
void  Send232Char(u8 t)        //发送数据，需要使能DMA发送通道，并且设置引脚为发送模式
{
	...
	RS232_TX_BUFF[len++]=t;                       //将需要发送的值赋值到DMA发送源地址
...
//	DMA_ClearITPendingBit(DMA1_IT_GL3); //清除全部中断标志
	modbus.recount = CMD_MAX_SIZE-DMA_GetCurrDataCounter(DMA1_Channel3); 

	for(i=0;i<modbus.recount;i++)
	{
		modbus.rcbuf[i] = RS232_RX_BUFF[i];
		#ifdef _TEST_DMA
		printf("%d\t%d\n",modbus.rcbuf[i],i);
		#endif
	}
