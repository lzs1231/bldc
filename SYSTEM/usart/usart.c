...
//	DMA_Init(DMA1_Channel5, &DMA_InitStructure);            //配置DMA1_Channel5
//	
//	DMA_ITConfig(DMA1_Channel5, DMA_IT_TC, ENABLE);         //DMA通道1传输完成中断
...
//	DMA_InitStructure.DMA_Priority = DMA_Priority_Medium ;    //优先级：高
//	DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;            //禁止内存到内存的传输
//	DMA_Init(DMA1_Channel4, &DMA_InitStructure);            //配置DMA1_Channel5
//	
//	DMA_ITConfig(DMA1_Channel4, DMA_IT_TC, ENABLE);         //DMA通道1传输完成中断
//	DMA_Cmd(DMA1_Channel4,ENABLE);
	
	
	USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);  //开启串口接受中断，接收一个字节，RXNE位被置位一次，产生一次中断
...
void Receive_DataPack(void)
{
	...
	/* 获取接收到的数据长度 单位为字节*/
	buff_length = CMD_MAX_SIZE - DMA_GetCurrDataCounter(DMA1_Channel5);        //设置DMA接收大小（一旦接收数据超过CMD_MAX_SIZE，就产生中断）-获取剩余字节空间 = 本次接收数据的大小
    /* 获取数据长度 */
    Usart_Rx_Sta = buff_length;
	/* 重新赋值计数值，必须大于等于最大可能接收到的数据帧数目 */
	DMA1_Channel5->CNDTR = CMD_MAX_SIZE;    
...
//		USART_ReceiveData(USART1);    //对USART_DR的读操作可以将接收中断标志位清零
//	}
